    ...
    async start(controller) {
      ...
      try {
        ...
        for (const agent of analysisAgents) {
          ...
          round1[agent.id] = analysis as Round1Analysis;
        ...
        if (message.includes('401') || message.includes('403')) {
          ...
          send({ type: 'error', message: 'Rate limited after retries. Please wait a minute and try again.' });
        } else {
...
// Throttle: wait between API calls to avoid rate limits
let lastCallTime = 0;
const MIN_CALL_INTERVAL = 1500; // ms between calls
...
async function callAgent(
  ...
  if (timeSinceLast < MIN_CALL_INTERVAL) {
    await sleep(MIN_CALL_INTERVAL - timeSinceLast);
  }
  ...
  try {
    ...
    if (isRateLimit && retryCount < 3) {
      ...
      const waitMs = 4000 * Math.pow(2, retryCount);
      await sleep(waitMs);
